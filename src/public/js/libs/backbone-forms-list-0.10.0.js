define(
		...
		function($, Backbone, Form) {
			...
			(function() {
				...
				/**
				 * LIST An array editor. Creates a list of other editor items. Special options:
				 * 
				 * @param {String}
				 *            [options.schema.itemType] The editor type for each item in the list.
				 *            Default: 'Text'
				 * @param {String}
				 *            [options.schema.confirmDelete] Text to display in a delete
				 *            confirmation dialog. If falsey, will not ask for confirmation.
				 */
				editors.List = editors.Base.extend({
					 ...
					 * 
					 * @param {Mixed}
					 *            [value] Value for the new item editor
					 */
				 ...
				 * 
				 * @param {editors.List}
				 *            options.list The List editor instance this item belongs to
				 * @param {Function}
				 *            options.Editor Editor constructor function
				 * @param {String}
				 ...
				 */
				editors.List.Item = Backbone.View
						.extend({
							events : {
							...
							validate : function() {
								var value = this.getValue(), formValues = this.list.form ? this.list.form.getValue()
				...
				/**
				 * Modal object editor for use with the List editor. To use it, set the 'itemType'
				 * property in a List schema to 'Object' or 'NestedModel'
				 */
				editors.List.Modal = editors.List.Object = editors.List.NestedModel = editors.Base.extend({
					 ...
					 *            options
					 * @param {Function}
					 *            [options.schema.itemToString] Function to transform the value for
					 *            display in the list.
					 * @param {String}
					 *            [options.schema.itemType] Editor type e.g. 'Text', 'Object'.
					 * @param {Object}
					 *            [options.schema.subSchema] Schema for nested form,. Required when
					 *            itemType is 'Object'
					 * @param {Function}
					 *            [options.schema.model] Model constructor function. Required when
					 *            itemType is 'NestedModel'
					 */
					initialize : function(options) {
						editors.Base.prototype.initialize.call(this, options);
					...
					itemToString : function(value) {
						...
						_.each(this.nestedSchema, function(schema, key) {
							var desc = schema.title ? schema.title : Form.helpers.keyToTitle(key), val = value[key];
					...
					/**
					 * Called when the user clicks 'OK'. Runs validation and tells the list when
