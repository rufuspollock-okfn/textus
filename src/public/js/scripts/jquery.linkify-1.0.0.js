// Linkify: A jQuery plugin that takes a section and turns it into a link. Version 1.00, 2008
// December, 21 by Dave Smith, www.dave-smith.info.
(function($) {
	$.fn.linkify = function(options) {
		var defaults = {
			...
			// Set the visited class, for example 'visited' would produce <div class="linkify
			// linkify-visited">
		...
		// Class that aids the checking for a visited link. CSS applied temporarily by Linkify:
		// a.linkify-check-visited:visited {overflow:auto;}
		...
		if ($('head style#linkify-style-element').length == 0) {
			...
			$('head').append(
					'<style id="linkify-style-element" type="text/css">a.' + settings.check_visited
							+ ':visited {overflow:auto;color:#f00;}</style>');
		}
		...
		return this
				.each(function() {
					...
					// With the nth section link, get its title, check for visited links and get the
					// uri.
					$('a', this).eq(settings.link_index).each(function() {
						// Check if the link has a title. The script assumes that the section had no
						// title attribute set.
						...
						if (settings.visited) {
							...
							if ($(this).css('overflow') == 'auto') {
								// Go through each section class and add a visited class based on
								// each.
								for ( var i = section_classes.length; --i > -1;) {
					...
					if (settings.focus) {
						...
						function mouseoverFocus() {
							...
							for ( var i = section_classes.length; --i > -1;) {
								...
								if (settings.visited
										&& $(section).hasClass(section_classes[i] + '-' + settings.visited_class)) {
									$(section).addClass(
											section_classes[i] + '-' + settings.visited_class + '-'
													+ settings.focus_class);
								}
							}
							window.status = uri;
						}
						...
						function mouseoutBlur() {
							...
							if ($(section).data(settings.store_name) == 0) {
								// Go through each section class and add a focus class based on
								// each.
								for ( var i = section_classes.length; --i > -1;) {
						...
						// Prepare some data for the section events. Assume that at initiation the
						// section had neither focus nor mouseover.
					...
					if (settings.focus_sub) {
						...
						function subMouseoverFocus() {
							...
							// Go through each section class and add a focus sub class based on
							// each.
							for ( var i = section_classes.length; --i > -1;) {
						...
						function subMouseoutBlur() {
							...
							if ($(this).data(settings.store_name) == 0) {
								// Go through each section class and remove a focus sub class based
								// on each.
								for ( var i = section_classes.length; --i > -1;) {
						...
						// Prepare some data for the links within a section. Assume that at
						// initiation the section had neither focus nor mouseover.
						// Apply the sub section mouseover/focus and mouseout/blur functions,
						// applies to the links within the section where the uri is not the indexed
						// link uri.
						$('a', this).filter(function() {
							return $(this).attr('href') != uri;
						}).data(settings.store_name, 0).mouseover(subMouseoverFocus).focus(subMouseoverFocus).mouseout(
								subMouseoutBlur).blur(subMouseoutBlur);
					}
