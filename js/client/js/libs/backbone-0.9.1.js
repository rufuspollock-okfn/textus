...
(function() {
	...
	// Ajax calls (a.k.a. the `$` variable). By default Backbone will use:
	// jQuery,
	...
	_.extend(Backbone.Model.prototype, Backbone.Events,
			{
				...
				set : function(key, value, options) {
					...
					for (attr in attrs) {
						...
						if (this._changing
								&& !_.isEqual(this._changed[attr], val)) {
					...
					// Fire the `"change"` events, if the model has been
					// changed.
					if (!alreadySetting) {
				...
				// you choose
				// to silence it. `unset` is a noop if the attribute doesn't
				// exist.
				...
				fetch : function(options) {
					...
					options.error = Backbone.wrapError(options.error, model,
							options);
					return (this.sync || Backbone.sync).call(this, 'read',
							this, options);
				},

				// Set a hash of model attributes, and sync the model to the
				// server.
				// If the server returns an attributes hash that differs, the
				// model's
				...
				// Optimistically removes the model from its collection, if it
				// has one.
				...
				destroy : function(options) {
					...
					options.error = Backbone.wrapError(options.error, model,
							options);
					var xhr = (this.sync || Backbone.sync).call(this, 'delete',
							this, options);
					if (!options.wait)
						triggerDestroy();
					return xhr;
				...
				// that will be called.
				url : function() {
					var base = getValue(this.collection, 'url')
							|| getValue(this, 'urlRoot') || urlError();
					if (this.isNew())
						return base;
					return base
				...
				// **parse** converts a response into the hash of attributes to
				// be `set` on
				// the model. The default implementation is just to pass the
				// response along.
				...
				// Call this method to manually fire a `"change"` event for this
				// model and
				// a `"change:attribute"` event for each changed attribute.
				// Calling this will cause all objects observing the model to
				// update.
				...
				// event.
				// If you specify an attribute name, determine if that attribute
				// has changed.
				...
				// determining what
				// parts of a view need to be updated and/or what attributes
				// need to be
				// persisted to the server. Unset attributes will be set to
				// undefined.
				// You can also pass an attributes object to diff against the
				// model,
				// determining if there *would be* a change.
				changedAttributes : function(diff) {
					if (!diff)
						return this.hasChanged() ? _.clone(this._changed)
								: false;
				...
				// Get the previous value of an attribute, recorded at the time
				// the last
				...
				// Check if the model is currently in a valid state. It's only
				// possible to
				...
				// Run validation against a set of incoming attributes,
				// returning `true`
				// if all is well. If a specific `error` callback has been
				// passed,
	...
	// or unordered. If a `comparator` is specified, the Collection will
	// maintain
	...
	_
			.extend(
					...
					Backbone.Events,
					{
						...
						// Add a model, or list of models to the set. Pass
						// **silent** to avoid
						...
						add : function(models, options) {
							...
							// Begin by turning bare objects into model
							// references, and preventing
							// invalid models or duplicate models from being
							// added.
							...
							// Listen to added models' events, and index models
							// for lookup by
							// `id` and by `cid`.
							for (i = 0; i < length; i++) {
							...
							// Insert models into the collection, re-sorting if
							// needed, and triggering
						...
						// any `add` or `remove` events. Fires `reset` when
						// finished.
						...
						// resetting the
						// collection when they arrive. If `add: true` is
						// passed, appends the
						...
						// Proxy to _'s chain. Can't be proxied the same way the
						// rest of the
						// underscore methods are proxied because it relies on
						// the underscore
						// constructor.
						chain : function() {
						...
						// Reset all internal state. Called when the collection
						// is reset.
						...
						// Prepare a model or hash of attributes to be added to
						// this collection.
						...
						// Internal method to remove a model's ties to a
						// collection.
			...
			.extend(
					...
					Backbone.Events,
					{
						...
						start : function(options) {
							...
							// `pushState`-enabled browser,
							// but we're currently in a browser that doesn't
							// support it...
						...
						// `replace: true`, if
						// you which to modify the current URL without adding an
						...
						navigate : function(fragment, options) {
							...
							if (this.fragment == frag
									|| this.fragment == decodeURIComponent(frag))
								return;
							...
							if (this._hasPushState) {
								...
								if (this.iframe
									...
									// and earlier to push a history entry on
									// hash-tag change.
	...
	_.extend(Backbone.View.prototype, Backbone.Events, {
		...
		// current view. This should be prefered to global lookups where
		// possible.
		$ : function(selector) {
		...
		// Remove this view from the DOM. Note that the view isn't present in
		// the
		...
		delegateEvents : function(events) {
			...
			for ( var key in events) {
				...
				if (!method)
					throw new Error('Event "' + events[key]
							+ '" does not exist');
